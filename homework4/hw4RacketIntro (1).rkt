; Type your name after the colon: Elan Rubin
...
(define (remove-last-helper L)
  (if (null? (rest L))
  null
  (cons (first L) (remove-last-helper (rest L)))))

(define (dropLast n L)
  (if (or (zero? n) (null? L))
  L
  (dropLast (- n 1) (remove-last-helper L))))
...
(define (stepways n)
  (cond
    [(> n 2) (+ (stepways (- n 1)) (stepways (- n 2)) (stepways (- n 3)))]
    [(> n 1) (+ (stepways (- n 1)) (stepways (- n 2)))]
    [(> n 0) (+ (stepways (- n 1)))]
    [else 1]))
